...
也可以作为机器学习入门练习。
                  该数据集是由Gorman等手机，并发布在一个名为palmerpenguins的R语言包，
以对南极企鹅种类进行分类和研究。
    ...
    st.header("预测企鹅分类")
    st.markdown("这个web应用是基于帕尔默群岛企鹅数据集建构的模型。只需要输入6个信息，就可以预测企鹅的物种，使用下面的表单开始预测吧！")

    col_form,col,col_logo=st.columns([3,1,2])
    with col_form:
        with st.form('user_inputs'):
            # 企鹅栖息的岛屿
            island = st.selectbox('企鹅栖息的岛屿', options=['托尔斯森岛', '比斯科普岛', '德里姆岛'])
            # 性别
